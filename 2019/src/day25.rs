...
fn main() {
    let program: Vec<i64> = read_to_string("inputs/day25.txt").unwrap()
        .split(",")
        .map(|s| {
        ...
        .collect();
    let pr: Vec<u8> = program.iter().map(|x| (*x).try_into().unwrap_or(32)).collect();
    println!("{:?}", String::from_utf8_lossy(&pr));
    let mut computer = Computer::new(&program);
    let mut command = String::new();
    let mut input: Vec<i64> = vec![];
    while !computer.halted { ;
        let output = computer.run(input.clone()).unwrap();
        let computer_says = String::from_utf8(output.iter().map(|x| (*x).try_into().unwrap()).collect()).unwrap();
        println!("Computer says: {}", computer_says);
        command.clear();
        std::io::stdin().read_line(&mut command);
        input = command.bytes().map(|x| x.try_into().unwrap()).collect();
