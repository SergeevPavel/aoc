...
impl Computer {
    ...
    pub(crate) fn run(&mut self, mut input: Vec<i64>) -> Result<Vec<i64>, String> {
        input.reverse();
...
pub fn read_program<P: AsRef<Path>>(path: P) -> Vec<i64> {
...
pub fn to_ascii(output: Vec<i64>) -> String {
    String::from_utf8(output.iter().map(|x| (*x).try_into().unwrap()).collect()).unwrap()
}